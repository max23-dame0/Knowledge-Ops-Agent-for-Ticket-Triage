...
| # | 任务 | 阶段 | 状态 | 下一步 | 阻塞 |
|:--:|------|------|:--:|------|------|
| 1 | 扩大覆盖评测（253 条全量：注入 60/良性 56 全量 + JailbreakBench 100 + OOS 30 + 域内 7，本地端点） | 已完成 | ✅ 已提交（863ddb0） | 注入 100% / Jailbreak 100% / OOS 100% 全绿 | 无 |
| 2 | 本地 API 接入（knot-proxy 优先，远程保留，--endpoint 切换） | 已完成 | ✅ 已提交（863ddb0，.env 本地配置不入库） | 工具型评测需 --endpoint remote | 无 |
