...
Pipeline (five controlled layers):

    L1 guardrails  -> hard safety rules (the ONLY deterministic overrule)
    L2 route_fn    -> deterministic route proposal with evidence (advisory)
    L3 plan        -> tool budget for the LLM (advisory, recorded in trace)
    L4 LLM         -> final decision on tools + answer + evidence
    L5 trace       -> structured decision record for replay and evaluation

Legacy precheck helpers (_resolve_route / _maybe_refuse / _maybe_clarify and
the *_looks_like_* probes) are kept as thin compatibility shims so existing
callers and tests keep working; their logic now lives in
src/agents/route_fn.py and src/agents/guardrails.py.
"""
...
from src.agents.contracts import AgentContract, RouteDecision, ToolPlan, ToolPlanStep
from src.agents.guardrails import evaluate_guardrails, looks_like_injection_attack
...
from src.agents.route_fn import (
    ...
    REFUSAL_SIGNALS,
    TICKET_HINTS,
    _extract_ticket_id as route_extract_ticket_id,
    _has_strong_escalation_signal as route_has_strong_escalation,
    _looks_like_escalation_policy_query as route_looks_like_escalation_policy,
    _looks_like_escalation_query as route_looks_like_escalation,
...
# Decision traces are appended per request for replay and offline evaluation.
_trace_recorder = TraceRecorder()

# Static contract shared with the evaluation harness (L5).
AGENT_CONTRACT = AgentContract(
    tool_names=("search_kb", "get_ticket_status", "create_escalation_draft"),
    output_schema="src.agents.main_agent.AgentAnswer",
    budget_limit=4,
    ...
    behavior_rules=(
        ...
        "no markdown headings or reasoning blocks in the final output",
    ),
)
...
class AgentAnswer(BaseModel):
    ...
    @field_validator("evidence", "next_action", "next_actions", mode="before")
    @classmethod
    def _coerce_list_fields(cls, value: Any) -> Any:
        """Tolerate model drift: accept strings/scalars where lists are expected."""
        if value is None:
            return []
...
def _build_plan(decision: RouteDecision, has_ticket_id: bool) -> ToolPlan:
    """Build the L3 tool budget for the proposed route."""
    if decision.route == "kb":
        return ToolPlan(
            steps=[ToolPlanStep(tool="search_kb", budget=1, reason="kb route: retrieve evidence")]
        )
    if decision.route == "ticket" and has_ticket_id:
        return ToolPlan(
            steps=[ToolPlanStep(tool="get_ticket_status", budget=1, reason="ticket route with id")]
        )
    if decision.route == "escalation":
        return ToolPlan(
            steps=[
                ToolPlanStep(
                    tool="create_escalation_draft",
                    budget=1,
                    reason="escalation route: draft suggestion",
                )
            ]
        )
...
def _run_agent_inner(user_input: str) -> AgentAnswer:
    """Execute the agent decision chain (guardrails + route + LLM) and normalize the output."""
    normalized = user_input.strip()
    logger.info("user_input=%s", normalized or "<empty>")

    # L1: hard guardrails (only deterministic layer that may overrule the LLM).
    guardrail = evaluate_guardrails(normalized)
    # L2: deterministic route proposal (advisory, carries evidence).
    route_decision = decide_route(normalized)
    ticket_id = _extract_ticket_id(normalized)
    # L3: tool budget plan.
    plan = _build_plan(route_decision, ticket_id is not None)

    trace = DecisionTrace(
        run_id=get_request_id() or uuid.uuid4().hex,
        question=normalized,
        guardrail=guardrail,
    ...
    logger.info(
        "route_hints=route:%s | ticket_id:%s | guardrail_blocked:%s | plan:%s",
        route_decision.route,
        ticket_id or "<none>",
        guardrail["blocked"],
        [step.tool for step in plan.steps] or "<none>",
    )

    # L1 hard refuse: deterministic, recorded in the trace.
    if guardrail["blocked"]:
        response = _finalize_response(_build_refusal_answer(), route="refuse")
        _record_trace(trace, response)
        logger.info("response_summary=guardrail_refusal | rules=%s", guardrail["rules_hit"])
        return response

    # Legacy deterministic clarification bridge (kept for behavioral parity;
    # will be migrated to LLM + eval-driven clarification in a later stage).
    clarification = _maybe_clarify(normalized)
    if clarification is not None:
        response = _finalize_response(clarification, route="clarify")
        _record_trace(trace, response)
    ...
    if not _circuit_breaker.allow_request():
        ...
        degraded = _finalize_response(_build_degraded_answer(route_decision.route), route=route_decision.route)
        _record_trace(trace, degraded)
        return degraded
    ...
    trace.llm = {
        ...
        "tool_call_count": len(_CURRENT_TOOL_CALLS),
        "budget_used": len(_CURRENT_TOOL_CALLS),
        "latency_ms": elapsed_ms,
        "raw_output": _truncate_for_trace(result.final_output),
    }
    _record_trace(trace, response)
    logger.info(
        "response_summary=route:%s | conclusion:%s | handoff:%s | confidence:%.2f | tool_calls:%s | latency_ms:%s",
        response.route,
        response.conclusion,
...
def _truncate_for_trace(value: Any, limit: int = 4000) -> str:
    """Return a bounded string view of the raw LLM output for traces."""
    if isinstance(value, str):
        text = value
...
def _record_trace(trace: DecisionTrace, response: AgentAnswer) -> None:
    """Attach the final response to the trace and persist it."""
    trace.final = response.model_dump()
    try:
        _trace_recorder.write(trace)
    except Exception as exc:  # noqa: BLE001 - tracing must never break the answer path
        logger.warning("trace_write_failed=%s", exc)


def _build_refusal_answer() -> AgentAnswer:
    """Return the deterministic refusal answer used by the hard guardrails."""
    return AgentAnswer(
...
def _build_degraded_answer(route: str) -> AgentAnswer:
    """Return the circuit-open degraded answer."""
    return AgentAnswer(
...
def _finalize_response(response: AgentAnswer, route: str) -> AgentAnswer:
    ...
    if tool_evidence:
        # Evidence derived from tool results is authoritative whenever tools ran.
...
# ---------------------------------------------------------------------------
# Legacy compatibility shims (logic moved to route_fn / guardrails).
# ---------------------------------------------------------------------------

# Re-exported signal tables (kept for any external importers).
REFUSAL_KEYWORDS = REFUSAL_SIGNALS


def _extract_ticket_id(user_input: str) -> str | None:
    """Extract ticket ids from flexible user input variants (see utils.ticket_id)."""
    return route_extract_ticket_id(user_input)


def _has_strong_escalation_signal(user_input: str) -> bool:
    """Return True for escalation phrasing that strongly prefers create_escalation_draft."""
    return route_has_strong_escalation(user_input)


def _looks_like_ticket_query(user_input: str) -> bool:
    """Return True when the request appears to ask about a ticket."""
    return route_looks_like_ticket(user_input)


def _looks_like_escalation_query(user_input: str) -> bool:
    """Return True when the request appears to ask for escalation advice."""
    return route_looks_like_escalation(user_input)
...
def _looks_like_kb_policy_query(user_input: str) -> bool:
    ...
    return route_looks_like_kb_policy(user_input)
...
def _looks_like_escalation_policy_query(user_input: str) -> bool:
    ...
    return route_looks_like_escalation_policy(user_input)


def _resolve_route(user_input: str, is_ticket_query: bool = False, is_escalation_query: bool = False) -> str:
    """Resolve the likely route before the agent runs (legacy signature kept).

    The extra arguments are ignored: route resolution now comes from
    src.agents.route_fn.decide_route, which recomputes the same signals.
    """
